    ...
    public static class ServerSettings
    {
        ...
        public class AI
        {
            ...
            public class CantReachSettings
            {
                ...
                @Comment("Deciding we can't reach the target requires at least this many ticks in combat")
                public int lastIdleThreshold = 1;

                @Config.Name("No Attack Time")
                @Comment("Deciding we can't reach the target requires at least this many ticks since the last successful attack")
                public int lastAttackThreshold = 200;

                @Config.Name("No Path Time")
                @Comment("Deciding we can't reach the target requires at least this many ticks without a path to the target")
                public int lastPathThreshold = 100;

                @Config.Name("Target Time")
                @Comment("Deciding we can't reach the target requires at least this many ticks with a valid target")
                public int lastNoTargetThreshold = 100;

                @Config.Name("Flee If Unreachable")
                @Comment("If we can't reach the target, do what brave Sir Robin does")
